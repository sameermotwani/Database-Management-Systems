...
hideOverlay()})})(jQuery);
(function($){$.doc=function(opt){this.defaults={'navStickyMinWidth':860,'mobile':!1,'docId':0}
this.navObj=$('#sub-header');this.headerObj=$('#l1-nav-container');this.bodyObj=$('body');this.footerObj=$('footer');this.docTocObj=$('#docs-sidebar-toc');this.docsMainObj=$('#docs-main');this.docsMainInner=$('#docs-main-inner');this.docTocInner=$('#docs-toc-inner');this.tocPosition=null;this.copyTimer=null;this.options=$.extend({},this.defaults,opt);this.init=function(){var self=this;$('.docs-sidebar-nav a.expandable').doctoc(self);$('#docs-hide-nav').click(function(e){self.docTocObj.hide();self.docsMainObj.removeClass('has-toc').addClass('no-toc');$('#docs-show-nav').show();self.docsMainObj.css('min-height','100px');$.get('/doc/sidebar/?visible=0&d='+self.options.docId,{});return !1});$('.docs-sidebar-section').click(function(e){var div=$(this).next();if(div.is(':hidden')){div.slideDown('fast',function(){self.adjustHeight()})}else{div.slideUp('fast',function(){self.adjustHeight()})}
...
return !1});$('.mediaobject img').each(function(){var width=$(this).attr('width');$(this).removeAttr('width').removeAttr('height');if(width>0){$(this).css('max-height','auto').css('width','100%').css('max-width',width+'px')}else{width=$(this).width();$(this).css('max-height','auto').css('width','100%').css('max-width',width+'px')}});$('#expand-footer').click(function(){var collapsed=1;var span=$('#footer-collapse').find('span');if($('footer').hasClass('collapsed')){collapsed=0;$('footer').removeClass('collapsed');$('#footer-links').show();span.removeClass('icon-chevron-up').addClass('icon-chevron-down');$(this).attr('title','Hide Footer');$('html, body').animate({scrollTop:$(document).height()},'slow')}else{$('#footer-links').hide();$('footer').addClass('collapsed');span.removeClass('icon-chevron-down').addClass('icon-chevron-up');$(this).attr('title','Expand Footer')}
$.get('/doc/footer/?collapsed='+collapsed,{});return !1});if(!this.options.mobile){$('pre.programlisting').hover(function(){var pre=this;if($(pre).find('.docs-select-all').length==0){var id='sa'+Math.floor((Math.random()*100000000)+1);var cmd=(navigator.appVersion.indexOf("Mac")!=-1)?'&#8984;+C':'CTRL+C';var div='<div class="docs-select-all"><div class="copy-help left">Press '+cmd+' to copy</div> <div class="right"><a href="" title="Select All" id="'+id+'"><span class="icon-clipboard"></span></a></div></div>'
$(pre).prepend($(div));$('#'+id).click(function(){var saDiv=$(pre).find('.docs-select-all')[0];var chDiv=$(pre).find('.copy-help')[0];if(window.getSelection){var selection=window.getSelection();var range=document.createRange();range.selectNodeContents(pre);range.setStartAfter(saDiv);selection.removeAllRanges();selection.addRange(range)}else if(document.body.createTextRange){var range=document.body.createTextRange();range.moveToElementText(pre);range.moveStart('character',$(saDiv).text().length);range.select()}
$(chDiv).fadeIn("slow",function(){self.copyTimer=window.setTimeout(function(){$(chDiv).fadeOut('slow')},4000)});return !1})}
$(pre).find('div:first').show();self.adjustHeight()},function(){window.clearTimeout(self.copyTimer);self.copyTimer=null;$(this).find('.copy-help').hide();$(this).find('div:first').hide();self.adjustHeight()})}
...
this.initComments=function(prefix){var self=this;$(prefix+' #docs-comment-show-form-btn').click(function(e){$(this).hide()
...
(function($){$.fn.loadVersion=function(ga){$('#'+ga).mask('Loading ...');var os=$('#os-'+ga).val();var url=$(this).attr('href');if(typeof url==='undefined'||url===!1){url=$('#dl-'+ga).attr('action');var version=$('#version-'+ga).val();var params={tpl:'version',os:os,version:version}}else{var params={tpl:'version',os:os}}
$.get(url,params).done(function(data){$('#'+ga).html(data);$('#'+ga).unmask()});return !1};$.fn.loadFiles=function(ga){var url=$('#dl-'+ga).attr('action');var os=$('#os-'+ga).val();if(os==0){return !1}
$('#files-'+ga).mask('Loading ...');var version=$('#version-'+ga).val();$.get(url,{tpl:'files',os:os,version:version}).done(function(data){$('#files-'+ga).html(data);$('#files-'+ga).unmask()});return !1};$.fn.loadArchiveVersion=function(){$('#downloads').mask('Loading ...');var os=$('#os').val();var url=$('#dl').attr('action');var version=$('#version').val();var params={tpl:'version',os:os,version:version};$.get(url,params).done(function(data){$('#downloads').html(data);$('#downloads').unmask()});return !1};$.fn.loadArchiveFiles=function(){var url=$('#dl').attr('action');var os=$('#os').val();if(os==0){return !1}
$('#files').mask('Loading ...');var version=$('#version').val();$.get(url,{tpl:'files',os:os,version:version}).done(function(data){$('#files').html(data);$('#files').unmask()});return !1};$.fn.showSignature=function(){var self=this;$.facebox(function(){var url=$(self).attr('href');$.get($(self).attr('href'),{tpl:'fb'}).done(function(data){$.facebox(data)})})}})(jQuery);
...
(function($){$.content=function(opt){this.defaults=$.extend({'main':0,'type':0},opt);this.init=function(){var self=this;$(window).hashchange(function(){self.load()})
...
$('#server_count').bind('keypress',function(e){if(e.keyCode==13){$(this).blur();return !1}})};this.refresh=function(top){var self=this;var msg=this.options.lead_captured?'Calculating':'Validating Selections';if(top){var new_position=$('#tco_graph').offset();window.scrollTo(new_position.left,new_position.top);var mask_id='#tco_graph'}else{var mask_id='#tco_form'}
...
(function(a){($.browser=$.browser||{}).mobile=/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(a)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0,4))})(navigator.userAgent||navigator.vendor||window.opera);(function($,sr){var debounce=function(func,threshold,execAsap){var timeout;return function debounced(){var obj=this,args=arguments;function delayed(){if(!execAsap)
func.apply(obj,args);timeout=null};if(timeout)
clearTimeout(timeout);else if(execAsap)
func.apply(obj,args);timeout=setTimeout(delayed,threshold||100)}}
$.fn[sr]=function(fn){return fn?this.bind('resize',debounce(fn)):this.trigger(sr)}})($,'smartresize');$(function(){var navStickyMinWidth=860;var ns={'svg':'http://www.w3.org/2000/svg'};if(!!document.createElementNS&&!!document.createElementNS(ns.svg,'svg').createSVGRect){if(!$('html').hasClass('svg')){$('html').addClass('svg')}}else{$('html').removeClass('svg');$('html').addClass('no-svg')}
...
if($('#l2-search-toggle').hasClass('open-search')){$('#l2-search-toggle').removeClass('open-search');$('#l3-search-container').removeClass('open-search')}else{$('#l2-search-toggle').addClass('open-search');$('#l3-search-container').addClass('open-search')}});if(!$.browser.mobile){var navObj=$('#sub-header');var bodyObj=$('body');var footerObj=$('footer');var headerObj=$('#l1-nav-container');var docTocObj=$('#docs-sidebar-toc');var navPosition=null;var stickyNav=function(){if(docTocObj.length){}else{if($(window).scrollTop()>headerObj.height()){var width=$(window).width();if((!navPosition||navPosition=='static')&&width>navStickyMinWidth){navObj.addClass('sticky');bodyObj.addClass('has-sticky-header').css('padding-top',navObj.height()+'px');navPosition='fixed'}else if(navPosition=='fixed'&&width<=navStickyMinWidth){navObj.removeClass('sticky');bodyObj.removeClass('has-sticky-header').css('padding-top','0px');navPosition='static'}}else{if(!navPosition||navPosition=='fixed'){navObj.removeClass('sticky');bodyObj.removeClass('has-sticky-header').css('padding-top','0px');navPosition='static'}}}};stickyNav();$(window).scroll(function(){stickyNav()})}
...
if($(this).width()<=navStickyMinWidth&&!created){setupSmallScreen()}});if(!$.browser.mobile&&!docTocObj.length){var scrollTo=function(hash){var newTop=null;var width=$(window).width();var offset=(width>navStickyMinWidth)?50:10;if($(hash).length){newTop=$(hash).offset().top-offset}else if($('a[name='+hash.substr(1)+']').length){newTop=$('a[name='+hash.substr(1)+']').offset().top-offset}
